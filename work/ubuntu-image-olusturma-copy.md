...
sudo parted $IMG --script \
  ...
  mkpart primary ext4 2049MiB 4097MiB \
...
sudo mkdir -p /mnt/ubuntu/sysrw
sudo mount ${LOOP}p2 /mnt/ubuntu/sysrw
...
/dev/vda2  /sysrw   ext4  rw,noatime                0  2

# User RW (resetlenebilir)
/dev/vda3  /data    ext4  rw,noatime                0  2

# Bind mounts (system)
/sysrw/etc        /etc        none  bind  0  0
/sysrw/var        /var        none  bind  0  0
/sysrw/root       /root       none  bind  0  0
/sysrw/usrlocal   /usr/local  none  bind  0  0
...
# User RW alanı – home
mkdir -p /mnt/ubuntu/data/home
ln -s /data/home /mnt/ubuntu/home

chroot /mnt/ubuntu groupadd -g 1000 myuser
chroot /mnt/ubuntu useradd \
  --uid 1000 \
  --gid 1000 \
  --home /home/myuser \
  --create-home \
...
echo "myuser:myuser123" | chroot /mnt/ubuntu chpasswd
chroot /mnt/ubuntu chown -R myuser:myuser123 /data/home/myuser

# Journald RAM-only
cat > /mnt/ubuntu/etc/systemd/journald.conf <<EOF
[Journal]
Storage=volatile
RuntimeMaxUse=8M
EOF

chroot /mnt/ubuntu systemctl disable \
  apt-daily.service \
  apt-daily.timer \
...
chroot /mnt/ubuntu systemctl disable getty@tty1.service
chroot /mnt/ubuntu systemctl enable serial-getty@ttyS0.service

# Gerekli Paketler (SPEC / PARSEC / gem5)
chroot /mnt/ubuntu apt install -y \
  build-essential \
  bash \
  coreutils \
  findutils \
...
# /var → RW Alan
chroot /mnt/ubuntu mkdir -p /data/var
chroot /mnt/ubuntu rm -rf /var
chroot /mnt/ubuntu ln -s /data/var /var

# Root Şifre
chroot /mnt/ubuntu passwd root

# System RW Alanlarının Dağıtılması
sudo mkdir -p /mnt/ubuntu/sysrw/{etc,var,root,usrlocal}

sudo mv /mnt/ubuntu/etc/*       /mnt/ubuntu/sysrw/etc/
sudo mv /mnt/ubuntu/var/*       /mnt/ubuntu/sysrw/var/
sudo mv /mnt/ubuntu/root/*      /mnt/ubuntu/sysrw/root/
sudo mv /mnt/ubuntu/usr/local/* /mnt/ubuntu/sysrw/usrlocal/

sudo mkdir -p /mnt/ubuntu/{etc,var,root,usr/local}
