...
class btb
{
    ...
    private:
        ...
        void insert_or_update(int pc, const btb_entry_t& entry, int tag_pc);
        bool is_direct_fillable(const btb_entry_record_t& entry) const;
        bool is_strongly_taken(const btb_entry_record_t& entry) const;
        btb_entry_record_t* find_branch_record(int pc,
                                               const bpu_sign_t& branch_sign);
        btb_commit_result_t update_branch_ctr(int pc,
                                              const bpu_sign_t& branch_sign,
                                              bool taken);
};
