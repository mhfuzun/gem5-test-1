...
int
ghistory::hash_foldedHistory(std::size_t start, std::size_t len,
                             int width) const
{
...
int
ghistory::hash_foldedHistory(std::size_t len, int width) const
{
    return hash_foldedHistory(0, len, width);
}

bool
ghistory::bit(std::size_t age) const
{
    if (bits.empty() || age >= bits.size())
        return false;

    const std::size_t idx = (head + age) % bits.size();
    return bits[idx] != 0;
}

void
ghistory::push(bool value)
{
    if (bits.empty())
        return;
