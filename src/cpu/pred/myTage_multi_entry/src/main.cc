...
[[maybe_unused]] static tage_cfg_t
make_cfg(int addrWidth, int comp_count, int depth, int history_width[],
         int tag_width[])
{
    tage_cfg_t cfg{};
    (void)addrWidth;
    ...
    cfg.pchistory_length = 0;
    cfg.comp_count = comp_count;
    cfg.entry_per_set = 1;
    cfg.fetch_line_bytes = 16;
    cfg.bimodal_ctrs_per_row = 4;
    cfg.bimodal_offset_shift = 2;
    cfg.bimodal_use_aligned_addr = true;
    cfg.tag_use_aligned_addr = true;
    ...
    cfg.periodicreset = false;
    cfg.periodicreset_branchperiod = 0;
    cfg.random_type = SIMRAND;
    cfg.lfsr_width = 1;
    cfg.lfsr_seed = 1;
    cfg.lfsr_misprediction_update = false;
    cfg.history_hash_type = HISTORY_HASH_FOLDED;
    cfg.replacement_mode = REPLACEMENT_USEFUL;
    cfg.useAltOnNA = false;
    cfg.numUseAltOnNa = 1;
    cfg.useAltOnNaBits = 4;
    cfg.useAltOnNaHashStart = 2;
    cfg.useAltOnNaHashWidth = 0;
...
read_cfg(string jsonfile)
{
    ...
    auto optional_bool = [&](const json& node, const char* key,
        ...
        throw std::runtime_error("field must be boolean-compatible: " +
                                 string(key));
    };
    ...
    cfg.entry_per_set = root.value("entry_per_set", 1);
    cfg.fetch_line_bytes = root.value("fetch_line_bytes", 16);
    cfg.bimodal_offset_shift = root.value("bimodal_offset_shift", 2);
    const int default_bimodal_ctrs =
        (cfg.bimodal_offset_shift >= 0 && cfg.bimodal_offset_shift < 63)
            ? static_cast<int>(
                  static_cast<std::uint64_t>(cfg.fetch_line_bytes) >>
                  cfg.bimodal_offset_shift)
            : 1;
    cfg.bimodal_ctrs_per_row =
        root.value("bimodal_ctrs_per_row",
                   default_bimodal_ctrs > 0 ? default_bimodal_ctrs : 1);
    cfg.bimodal_use_aligned_addr =
        optional_bool(root, "bimodal_use_aligned_addr", true);
    cfg.tag_use_aligned_addr =
        optional_bool(root, "tag_use_aligned_addr", true);
    ...
    cfg.allocate_randomplacement =
        require_int(root, "allocate_randomplacement");
    ...
    const string random_type = root.value("random_type", string("SIMRAND"));
    ...
    const string history_hash_type = root.value(
        "history_hash_type", root.value("history_hash", string("FOLDED")));
    if (history_hash_type == "FOLDED") {
        cfg.history_hash_type = HISTORY_HASH_FOLDED;
    } else if (history_hash_type == "CSR" ||
               history_hash_type == "CIRCULAR_SHIFT_REGISTER") {
        cfg.history_hash_type = HISTORY_HASH_CSR;
    } else {
        throw std::runtime_error(
            "history_hash_type must be FOLDED, CSR, or "
            "CIRCULAR_SHIFT_REGISTER");
    }

    const string replacement_mode =
        root.value("replacement_mode", string("USEFUL"));
    if (replacement_mode == "USEFUL" || replacement_mode == "USEFULL") {
        cfg.replacement_mode = REPLACEMENT_USEFUL;
    } else if (replacement_mode == "PLRU") {
        cfg.replacement_mode = REPLACEMENT_PLRU;
    } else {
        throw std::runtime_error("replacement_mode must be USEFUL or PLRU");
    }

    cfg.useAltOnNA = optional_bool(
        root, "useAltOnNA",
        optional_bool(root, "useAltPredForNewlyAllocated", false));
    cfg.numUseAltOnNa = root.value("numUseAltOnNa", 1);
    cfg.useAltOnNaBits = root.value("useAltOnNaBits", 4);
    cfg.useAltOnNaHashStart = root.value("useAltOnNaHashStart", 2);
    cfg.useAltOnNaHashWidth = root.value("useAltOnNaHashWidth", 0);
...
getTraceList(const string& file_path, int core_id, std::size_t max_count)
{
    ...
    while (std::getline(fin, line)) {
        ...
        // core   0: 0x0000000080001050 0 0x000000008000105c
        const int parsed =
            std::sscanf(line.c_str(), "core %d: 0x%llx %d 0x%llx",
    ...
    LOG("trace parse done, lines=" << line_no
                                   << ", entries=" << traces.size());
    return traces;
}

static int
trace_loop(const app::Config& config)
{
    /*
    int addrWidth = 32;
    ...
    */
    ...
    cout << "Accuracy        : " << acc << "%\n";
    cout << "misp/KI         : "
         << (double)(total - correct) / (total / 1000.0) << " misp/KI\n";
