...
template <typename T>
static void checkVectorSize(const std::vector<T> &vec, unsigned expected,
                            const char *name) {
  if (vec.size() != expected) {
    fatal(
        "MyTAGE config error: '%s' size (%u) must equal compCount (%u)",
...
tage_cfg_t MyTAGE::buildCfg(const MyTAGEParams &params) {
...
statistics_t MyTAGE::aggregateBackendStats() const {
  ...
  for (const auto &backend : backendByThread) {
    ...
    aggregate.noAllocationBecauseOfnoFree += stats.noAllocationBecauseOfnoFree;
    aggregate.noAllocationBecauseOfproviderHigh +=
        stats.noAllocationBecauseOfproviderHigh;
    aggregate.noAllocationButProviderZero += stats.noAllocationButProviderZero;
...
bool MyTAGE::lookup(ThreadID tid, Addr pc, void *&bp_history) {
  assert(tid < backendByThread.size());

  auto *hist = new MyTAGEHistory();
  hist->isConditional = true;
  hist->predTaken = backendByThread[tid]->getPrediction(pc, hist->tageState);
  bp_history = hist;

  return hist->predTaken;
}

void MyTAGE::updateHistories(ThreadID tid, Addr pc, bool uncond, bool taken,
  ...
  if (bp_history == nullptr) {
    ...
    if (uncond) {
      backendByThread[tid]->snapshotState(hist->tageState);
    } else {
      backendByThread[tid]->getPrediction(pc, hist->tageState);
    }
    bp_history = hist;
  }

  auto *hist = static_cast<MyTAGEHistory *>(bp_history);
  backendByThread[tid]->updateHist(pc, taken, hist->tageState);
  myStats.speculativeHistoryUpdates++;
}

void MyTAGE::update(ThreadID tid, Addr pc, bool taken, void *&bp_history,
  ...
  if (squashed) {
    backendByThread[tid]->resetState(hist->tageState);
    backendByThread[tid]->updateHist(pc, taken, hist->tageState);
  ...
  if (hist->isConditional) {
    backendByThread[tid]->update(pc, taken, hist->tageState);
...
void MyTAGE::squash(ThreadID tid, void *&bp_history) {
  assert(tid < backendByThread.size());
...
void MyTAGE::branchPlaceholder(ThreadID tid, Addr pc, bool uncond,
                               void *&bp_history) {
  assert(tid < backendByThread.size());
...
void MyTAGE::MyTAGEStats::regStats() {
...
void MyTAGE::MyTAGEStats::preDumpStats() {
